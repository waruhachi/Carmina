...
extension ColorPoint: CustomStringConvertible {
    var description: String {
        String(
            format: "Point(x=%.3f,y=%.3f,c=\(UIColor(color).hex))",
            position.x,
            position.y
        )
    }
...
extension ColorPoint: Animatable {
    typealias AnimatableData = AnimatablePair<
        UnitPoint.AnimatableData, Color.Resolved.AnimatableData
    >
...
extension ColorPoint {
    fileprivate static var zero: ColorPoint {
        ColorPoint(position: .zero, color: .black.opacity(0))
    }

    fileprivate init(_ animatableData: ColorPoint.AnimatableData) {
        self.init(
...
extension Color {
    fileprivate init(_ animatableData: Color.Resolved.AnimatableData) {
...
extension UnitPoint {
    fileprivate static let animatableDataRatio =
        UnitPoint(x: 1, y: 1).animatableData.first / UnitPoint(x: 1, y: 1).x

    fileprivate init(_ animatableData: UnitPoint.AnimatableData) {
        self.init(
...
extension ColorPoints {
    init(points: [ColorPoint]) {
        self.init(
            point0: points[safe: 0]
                ?? ColorPoint(position: .zero, color: .black.opacity(0)),
            point1: points[safe: 1]
                ?? ColorPoint(position: .zero, color: .black.opacity(0)),
            point2: points[safe: 2]
                ?? ColorPoint(position: .zero, color: .black.opacity(0)),
            point3: points[safe: 3]
                ?? ColorPoint(position: .zero, color: .black.opacity(0)),
            point4: points[safe: 4]
                ?? ColorPoint(position: .zero, color: .black.opacity(0)),
            point5: points[safe: 5]
                ?? ColorPoint(position: .zero, color: .black.opacity(0)),
            point6: points[safe: 6]
                ?? ColorPoint(position: .zero, color: .black.opacity(0)),
            point7: points[safe: 7]
                ?? ColorPoint(position: .zero, color: .black.opacity(0))
        )
    }
...
extension ColorPointsAnimatableData: VectorArithmetic {
    static func - (
        lhs: ColorPointsAnimatableData,
        rhs: ColorPointsAnimatableData
    ...
    var magnitudeSquared: Double {
        value0.magnitudeSquared + value1.magnitudeSquared
            + value2.magnitudeSquared + value3.magnitudeSquared
            + value4.magnitudeSquared + value5.magnitudeSquared
            + value6.magnitudeSquared + value7.magnitudeSquared
    }
