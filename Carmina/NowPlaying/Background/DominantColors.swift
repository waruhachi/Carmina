...
extension UIImage {
    func dominantColorFrequencies(
        ...
        let image =
            cgImage?.colorSpace?.model == .rgb ? self : convertToRGBColorspace()
        let maxNumberOfColors = 500
        let targetSize = quality.targetSize(for: resolution)
        guard
            let colorCounts = image?
        ...
        let similarColors = mergeSimilar(
            colors: Array(colorCounts),
            diffThreshold: 6,
            maxCount: 20
        )
...
private func mergeSimilar(
    colors: [ColorCount],
    diffThreshold: CGFloat = 10.0,
    maxCount: Int
) -> [ColorCount] {
    ...
    for colorCount in colors {
        ...
        for dominantColor in result {
            let differenceScore =
                CGFloat(
                    colorCount.color.lab.deltaECIE94(
                        rhs: dominantColor.color.lab
                    )
                )
...
extension CGImage {
    fileprivate func colorCounts(maxAlpha: UInt8) -> [ColorCount]? {
...
extension Color {
    init(rgb: RGBBytes) {
        ...
        lab = Lab(
            xyz: XYZ(
                r: (CGFloat(rgb.r) / 255).rounded(
                    .toNearestOrEven,
                    precision: 100
                ),
                g: (CGFloat(rgb.g) / 255).rounded(
                    .toNearestOrEven,
                    precision: 100
                ),
                b: (CGFloat(rgb.b) / 255).rounded(
                    .toNearestOrEven,
                    precision: 100
                )
            ).rounded(.toNearestOrEven, precision: 100)
...
extension CGFloat {
    fileprivate func rounded(_ rule: FloatingPointRoundingRule, precision: Int)
        -> CGFloat
    {
...
extension CGSize {
    fileprivate var pixelCount: CGFloat {
        return width * height
    }

    fileprivate func transformToFit(in targetPixelCount: CGFloat) -> CGSize {
        let ratio = pixelCount / targetPixelCount
        let targetSize = CGSize(
            width: width / sqrt(ratio),
            height: height / sqrt(ratio)
        )
